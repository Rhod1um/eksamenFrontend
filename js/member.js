...
function createTable(fetchedList) {
    for (const object of fetchedList) { //for of loop
        const html = `
        <tr>
            <td>${object.memberId}</td> <!--husk skal stå som i java-->
            <td>${object.name}</td>
            <td>${object.email}</td>
            <td>
...
function showUpdateDialog(object){
    ...
    selectedId = object.memberId //"gemmer" id på selected teacher når man trykker update
...
function updateSubmit(event){
    ...
    update(selectedId, name, email)
    document.querySelector("#dialog-update").close()
}
//send PUT
async function update(memberId, name, email){
    const object = {memberId, name, email} //husk rigtig id navn når json objekt laves!! ellers 404
