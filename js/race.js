...
async function updateTable(){
    ...
    sortByDate() //gøres før noget vises i tabel, så det altid er sorteret alfabetisk
...
function updateSubmit(event){
    ...
    console.log(date)
    update(selectedId, date)
    document.querySelector("#dialog-update").close()
}
//send PUT
async function update(raceId, date){
    const object = {raceId: raceId, date: date} //husk rigtig id navn når json objekt laves!! ellers 404
...
//sorter alfabetisk
function sortByDate(){ //hvis vi ikke sortere så sorteres det på mærkelig måde af firebase
    fetchedList.sort((object1, object2) => object1.date.localeCompare(object2.date))
